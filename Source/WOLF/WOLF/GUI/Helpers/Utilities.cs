...
namespace WOLF
{
    public static class Utilities
    {
        ...
        static readonly double HOURS_PER_DAY = (GameSettings.KERBIN_TIME) ? 6.0 : 24.0;
        ...
        {
            if (config.HasValue(name) && double.TryParse(config.GetValue(name), out double newValue))
            {
                return newValue;
            }
            else
